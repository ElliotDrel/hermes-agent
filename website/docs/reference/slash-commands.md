...
| `/rename [title]` | **Discord threads only.** Regenerate the title from conversation history when no title is supplied, or set the supplied title directly. Updates both the session title and visible thread name. |
| `/resume [name]` | Resume a previously named session. |
