...
| `DISCORD_COMMAND_SYNC_POLICY` | Discord slash-command sync policy: `safe` (diff and reconcile), `bulk` (legacy `tree.sync()`), `startup` (sync once per gateway process, including after restarts), or `off` |
